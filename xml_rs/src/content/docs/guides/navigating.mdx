...
Each finder has a namespaced (`_ns`) counterpart that matches the full
`prefix:tag` name instead of just the local name:

```rust
// Match "t:book" rather than any "book"
let first = document.find_first_child_ns(root_id, "t:book")?;
let all = document.find_all_child_ns(root_id, "t:book")?;
```
